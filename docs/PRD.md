...
11. **账号登录（邮箱 / 密码）**
    - 仅用于账号身份认证与 Pro 功能解锁，**数据完全不同步**
    - 云端只存账号身份信息（userId、email、密码哈希、会员状态），绝不上传任何软件清单、使用记录、AI 密钥
    - 未登录 / 无网时，软件管理、统计等本地功能照常可用
    - 登录 Token 经主进程 `safeStorage` 加密落盘，不进 localStorage

#### 登录模块数据归属

| 存储位置 | 数据项 |
|---------|--------|
| 云端（账号服务器） | userId、email、密码哈希（bcrypt/argon2）、邮箱验证状态、昵称/头像、会员状态 |
| 本地 · 安全存储 | accessToken / refreshToken、Token 过期时间 |
| 本地 · 配置 | 上次登录邮箱、「记住我」、脱敏资料缓存、登录态布尔值 |
| 本地 · SQLite/主进程 | 软件清单、使用时长、session、AI 配置 + apiKey、偏好设置 |
| 绝不上云（红线） | 明文密码、登录 Token、AI apiKey、软件清单、使用行为数据 |

> 详细字段、IPC 通道与安全要求见《技术架构文档》第 6.5 节。
