...
const KIND_META: Record<ShareKind, { label: string; icon: typeof WorkflowIcon; color: string }> = {
  workflow: { label: '工作流', icon: WorkflowIcon, color: '#8b5cf6' },
  favorite_group: { label: '收藏夹', icon: Star, color: '#f59e0b' },
...
function getStatusInfo(s: MyShare): { label: string; className: string } {
  if (s.isRevoked) return { label: '已撤销', className: 'bg-slate-700/40 text-slate-500' };
  if (s.isArchived) return { label: '已归档', className: 'bg-slate-700/40 text-slate-500' };
  if (s.expiresAt && new Date(s.expiresAt).getTime() < Date.now()) {
    return { label: '已过期', className: 'bg-slate-700/40 text-slate-500' };
  }
  return { label: '生效中', className: 'bg-emerald-500/15 text-emerald-300' };
}
            ...
            return (
              ...
              >
                ...
                {confirmDelete === share.id && (
                  <div className="mt-3 flex items-center justify-between gap-3 text-xs rounded-lg px-3 py-2 bg-rose-500/10 text-rose-300">
