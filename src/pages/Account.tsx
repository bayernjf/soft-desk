...
export function Account() {
  ...
  if (loggedIn && profile) {
    ...
    return (
      <div className="space-y-8">
        <div>
          <h1 className="text-2xl font-bold text-white tracking-tight">账号</h1>
          <p className="text-sm text-slate-500 mt-1">管理你的登录账号与会员状态</p>
        </div>

        <div className="max-w-2xl rounded-2xl border border-slate-800/60 bg-[#0d0d14]/80 p-6">
          ...
          <div className="mt-6 grid sm:grid-cols-2 gap-3 text-sm">
            <div className="rounded-xl bg-slate-800/30 p-3">
              <div className="text-xs text-slate-500">邮箱验证</div>
              <div className="mt-1 flex items-center gap-1.5 text-slate-200">
                <ShieldCheck
                  className={cn('w-4 h-4', profile.emailVerified ? 'text-emerald-400' : 'text-slate-500')}
                />
                {profile.emailVerified ? '已验证' : '未验证'}
              </div>
            </div>
            <div className="rounded-xl bg-slate-800/30 p-3">
              <div className="text-xs text-slate-500">上次登录</div>
          ...
          <button
            onClick={() => logout()}
            className="mt-6 inline-flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-medium text-rose-300 bg-rose-500/10 border border-rose-500/20 hover:bg-rose-500/20 transition-colors"
          >
        ...
        <div className="max-w-2xl flex items-start gap-2 text-xs text-slate-500 leading-relaxed">
          <CloudOff className="w-4 h-4 shrink-0 mt-0.5 text-slate-600" />
          <p>
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="max-w-md rounded-2xl border border-slate-800/60 bg-[#0d0d14]/80 p-6">
        <div className="flex p-1 rounded-xl bg-slate-800/40 mb-6">
      ...
      <div className="max-w-md flex items-start gap-2 text-xs text-slate-500 leading-relaxed">
        <CloudOff className="w-4 h-4 shrink-0 mt-0.5 text-slate-600" />
        <p>
  ...
  return (
    <div className="relative">
      ...
      <input
        ...
        className="w-full pl-9 pr-3 py-2.5 rounded-xl bg-slate-800/40 border border-slate-700/60 text-sm text-slate-200 placeholder:text-slate-500 focus:outline-none focus:border-violet-500/60 focus:ring-2 focus:ring-violet-500/20 transition-all"
      />
    </div>
  );
}
