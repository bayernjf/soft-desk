...
import { searchSoftwareByIntent } from '@/services/ai.service';
...
/**
 * 自然语言语义搜索 hook(强回退 + 省钱):
 * - 防抖 DEBOUNCE_MS 后再触发,避免逐字调用;
 * - 仅当存在启用模型(aiReady) 且查询像自然语言意图时才调模型;
 * - 同一查询走内存缓存,不重复请求;
...
export function useSemanticSearch(
  ...
  useEffect(() => {
    ...
    if (!isElectron || !aiReady || !looksLikeIntent(q, localHitCount)) {
      setState({ ids: null, loading: false, fromAi: false });
      return;
    }

    const cached = cacheRef.current.get(q);
    if (cached) {
      setState({ ids: cached, loading: false, fromAi: true });
      return;
    }
    ...
    const timer = setTimeout(() => {
      searchSoftwareByIntent(q, software)
        .then((ids) => {
          if (cancelled) return;
          if (ids === null) {
            setState({ ids: null, loading: false, fromAi: false });
          ...
          setState({ ids, loading: false, fromAi: true });
        })
        .catch(() => {
          if (!cancelled) setState({ ids: null, loading: false, fromAi: false });
        });
    }, DEBOUNCE_MS);
    ...
    // software 变动不应重新触发(避免扫描刷新导致重复调用);仅查询/模型状态/本地命中数变化时触发
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [query, isElectron, aiReady, localHitCount]);
