...
create table if not exists share_reports (
  id                bigint generated by default as identity primary key,
  share_id          bigint not null references shares(id) on delete cascade,
  reporter_id       text not null,                                 -- 举报者 user_id, 必须登录才能举报(反匿名刷单)
...
begin
  -- 按 distinct reporter_id 计数, 与"独立举报人数"语义对齐。
  -- 即便下游 unique 索引拦截前已经存在旧脏数据, 这里也能自动收敛到正确值。
...
-- 用 Supabase Auth 原生 auth.uid() 而非 current_setting('app.current_user_id'),
-- 与项目内 radial_configs 表策略保持一致 (客户端未设置 session 变量,
-- 用 current_setting 会因 NULL 匹配失败导致 RLS 违规).
...
-- 策略 1: 只有已登录用户可插入举报, 且 reporter_id 必须是自己
-- (0.1 版曾允许 with check (true) 让匿名也能举报, 但这会打开无限刷单口子,
-- 加上 reporter_id NOT NULL + unique(share_id,reporter_id) 后, 这里也必须收紧)
